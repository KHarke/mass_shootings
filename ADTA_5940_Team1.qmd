...
summary_victims <- victims %>%
  group_by(B_CaseNum) %>%
  summarise(V_avgAge = mean(Age),
            V_minAge = min(Age),
            V_avgYrsLost = mean(as.integer(Years.Lost)),
            V_whitePercentage = sum(Race == 0) / n() * 100,
            #V_whitecount = sum(Race == 0),
            V_totalcount = n(),
            V_blackPercentage = sum(Race == 1) / n() * 100,
            V_malePercentage = sum(Gender == 0) / n() * 100,
            #V_malecount = sum(Gender == 0),
            #V_femalePercentage = sum(Gender == 1) / n() * 100,
            #V_femalecount = sum(Gender == 1),
            V_knewShooterPercentage = sum(Knew.Shooter == 1) / n() * 100,
            #V_knewShootercount = sum(Knew.Shooter == 1)
            )
summary_victims
```

Now we'll join the summary of victims to the Full_Database based on case number.

```{r}
Full_Database<- left_join(Full_Database, summary_victims, by="B_CaseNum")
str(Full_Database)
head(Full_Database)
```
...
We can create a list of variables to remove. This can be tweaked.

```{r}
# Domain Knowledge - remove columns not relevant to the problem

# Columns we may want to remove from columns_to_remove: "OB_Age", "OB_Gender", "OB_Race", "L_ArmedPersononScene",, "L_Region", "L_Location",   "L_StateCode" "L_InsiderorOutsider","L_WorkplaceShooting", "D_Year","OB_Immigrant","OB_EmploymentStatus","OB_EmploymentType", "OB_MilitaryService","CV_KnowntoPoliceorFBI", "CV_CriminalRecord", "CV_PartICrimes", "CV_PartIICrimes", "CV_HighestLevelofJusticeSystemInvolvement","W_InterestinFirearms", "W_FirearmProficiency", "W_TotalFirearmsBroughttotheScene",

# Remove columns
columns_to_remove <- c("B_ShooterLastName", "B_ShooterFirstName", "D_FullDate", 
"D_DayofWeek", "D_Day", "D_Month",  "L_StreetNumber", "L_StreetName", "L_City",
"L_County", "L_ZipCode", "L_Latitude", "L_Longitude"
, "L_MultipleLocations","L_OtherLocation","L_SpecifyArmedPerson",
 "OB_Height", "OB_Weight", "OB_Age", "OB_Gender", "OB_Race", "L_ArmedPersononScene", "L_Region", "L_Location",   "L_StateCode", "L_InsiderorOutsider","L_WorkplaceShooting", "D_Year","OB_Immigrant","OB_EmploymentStatus","OB_EmploymentType", "OB_MilitaryService","CV_KnowntoPoliceorFBI", "CV_CriminalRecord", "CV_PartICrimes", "CV_PartIICrimes", "CV_HighestLevelofJusticeSystemInvolvement","W_InterestinFirearms", "W_FirearmProficiency", "W_TotalFirearmsBroughttotheScene",
...
"OB_YoungerSiblings", "OB_RelationshipStatus", "OB_Children",  "OB_MilitaryBranch", "OB_CommunityInvolvement",
"OB_CommunityInvolvementSpecified", 
...
# From KH: This code gave this error: missing value where TRUE/FALSE needed
...
# Step 3: Visualize the correlation matrix
corrplot(correlation_matrix, method = "color", tl.cex=0.3, addCoef.col = "black")
...
#Full_Database_noCorrVars <- Full_Database %>% select(-one_of(highly_correlated_vars))
