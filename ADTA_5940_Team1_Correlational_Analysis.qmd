...
Correlation: We can delete strongly correlated columns, as a result, our dataset will have less redundancy.
...
"L_County", "L_ZipCode", "L_Latitude", "L_Longitude"
, "L_StateCode"
, "L_Region",
...
,"GM_KnownPrejudices"
,"GM_Motive_Racism_Xenophobia"
, "GM_Motive_ReligiousHate"
, "GM_Motive_Misogyny"
,"GM_Motive_Homophobia"
, "GM_Motive_EmploymentIssue"
, "GM_Motive_EconomicIssue"
, "GM_Motive_LegalIssue"
,"GM_Motive_RelationshipIssue"
, "GM_Motive_InterpersonalConflict"
, "GM_Motive_Fame_Seeking"
, "GM_Motive_Other"
,"GM_Motive_Unknown"
, "GM_RoleofPsychosisintheShooting"
, "SOC_SocialMediaUse", "SOC_Leakage",
...
print(Missing_values_cols_to_remove)
```
...
#results show low variance for V_FamilyMemberVictim.
```
...
print(numeric_vars)# this provides
...
# Step 2: Compute correlation matrices using different methods
...
# Step 3: Visualize the correlation matrices
corrplot(cor_matrix_pearson, method = "color", addCoef.col = "black", title = "Pearson Correlation")
corrplot(cor_matrix_spearman, method = "color", addCoef.col = "black", title = "Spearman Correlation")
corrplot(cor_matrix_kendall, method = "color", addCoef.col = "black", title = "Kendall Correlation")

# Step 4: Identify highly correlated variables
highly_correlated_pearson <- findCorrelation(cor_matrix_pearson, cutoff = 0.7)
highly_correlated_vars_pearson <- colnames(numeric_vars)[highly_correlated_pearson]

highly_correlated_spearman <- findCorrelation(cor_matrix_spearman, cutoff = 0.7)
highly_correlated_vars_spearman <- colnames(numeric_vars)[highly_correlated_spearman]

highly_correlated_kendall <- findCorrelation(cor_matrix_kendall, cutoff = 0.7)
highly_correlated_vars_kendall <- colnames(numeric_vars)[highly_correlated_kendall]

# Step 5: Remove one of the pair of highly correlated variables from your data
selected_features <- Full_Database %>% select(-one_of(
  highly_correlated_vars_pearson,
  highly_correlated_vars_spearman,
  highly_correlated_vars_kendall
))

# Now 'selected_features' dataset contains your selected features for further analysis
Full_Database <- selected_features
...
# Step 1: Convert character variables to factors
character_vars <- Full_Database %>% select_if(is.character)
Full_Database <- Full_Database %>% mutate(across(all_of(names(character_vars)), as.factor))

# Step 2: Identify numeric variables
numeric_vars <- Full_Database %>% select_if(is.numeric)

# Step 3: Compute correlation matrix
correlation_matrix <- correlate(numeric_vars, use = "pairwise.complete.obs")

# Step 4: Visualize the correlation matrix
corrplot(correlation_matrix, method = "color", addCoef.col = "black")

# Step 5: Identify highly correlated variables
highly_correlated <- correlate_select(correlation_matrix, threshold = 0.7, method = "pearson")$colname

# Step 6: Remove highly correlated variables from your data
selected_features <- Full_Database %>% select(-one_of(highly_correlated))
Full_Database <- selected_features
